/**
 * Migration script: Import Pittsburgh CSV data into Firebase Firestore
 * 
 * Usage: node migrate-pittsburgh.mjs
 * 
 * CSV columns:
 *   0: Address, 1: City, 2: State, 3: Machine/%
 *   4: Date, 5: Total weight, 6: Half weight
 *   7: User (header mislabels as 50$)
 *   8: 50$, 9: 20$, 10: 10$, 11: 5$, 12: 1$/watches
 * 
 * Cleans up previous "Pittsburgh" entries, then uploads fresh.
 */
...
const ZONE = 'Pittsburgh';

const CSV_PATH = '/Users/libi/Downloads/Vending Book - Pittsburgh .csv';
...
function isStreetAddress(str) {
    ...
    if (/^(put|make|text|call|change|cheng|mechine|machine|the[ay]\s|every|close|told|somebody|building|picked|no\s|token|go\s|always|add\s|Mr\.|Saif|opening|changed)/i.test(s)) return false;
    if (/^new\s/i.test(s)) return false;
    if (!/^\d/.test(s)) return false;
    if (/\b(st|street|ave?|avenue|blvd|boulevard|rd|road|ln|lane|dr|drive|pl|place|way|ct|court|hwy|highway|pike|pkwy|parkway|broadway|plaza)\b/i.test(s)) return true;
    return false;
}
...
function isNoMoney(totalWeight) {
    ...
    return /no\s*money/i.test(tw) || /^closed$/i.test(tw) || /^installed$/i.test(tw) || /^broken$/i.test(tw);
}
...
function isInactiveLocation(notes) {
    for (const n of notes) {
        ...
        if (/machine\s*out\b/i.test(lower)) return true;
        if (/\bin\s*storage\b/i.test(lower)) return true;
    }
    return false;
}
...
console.log(`\n📊 Parsed ${locationGroups.length} locations from Pittsburgh CSV\n`);
...
for (let i = 0; i < locationGroups.length; i++) {
    ...
    const subtitleParts = loc.notes.filter(n => {
        ...
        return (
            ...
            /new\s*install/i.test(n) || /installed/i.test(n) ||
            /storage/i.test(n) || /move/i.test(n) || /bracket/i.test(n) ||
            /opening/i.test(n) || /owner/i.test(n) || /always/i.test(n) ||
            /every\s*\d/i.test(n) || /out\b/i.test(n)
        );
    });
