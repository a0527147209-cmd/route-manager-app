/**
 * Migration script: Import Queens CSV data into Firebase Firestore
 * 
 * Usage: node migrate-queens.mjs
 * 
 * Queens CSV columns:
 *   0: Address, 1: Name (business), 2: City, 3: State, 4: Machine/%
 *   5: Date, 6: Total weight, 7: Half weight, 8: (user), 9: 20$, 10: 10$, 11: 5$, 12: 1$, 13: Notes
 * 
 * ADDS to existing data (does NOT delete).
 */

import { readFileSync } from 'fs';
import { initializeApp } from 'firebase/app';
import { getFirestore, collection, addDoc, getDocs, deleteDoc, doc } from 'firebase/firestore';

const firebaseConfig = {
    apiKey: "AIzaSyBUvIArE56nvLDmtaIJfz88XqdOzXqO44o",
    authDomain: "vending-book.firebaseapp.com",
    projectId: "vending-book",
    storageBucket: "vending-book.firebasestorage.app",
    messagingSenderId: "622650100879",
    appId: "1:622650100879:web:cae923c3c6590fc0d974fa",
};

const app = initializeApp(firebaseConfig);
const db = getFirestore(app);
const COLLECTION = 'customers';

const CSV_PATH = '/Users/libi/Downloads/Vending Book - Queens.csv';
const raw = readFileSync(CSV_PATH, 'utf-8');
const lines = raw.split('\n').map(l => l.replace(/\r$/, ''));
const dataLines = lines.slice(1);

function isStreetAddress(str) {
    const s = str.trim();
    if (!s) return false;
    // Reject phone numbers (7+ consecutive digits at start)
    if (/^\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}/.test(s)) return false;
    if (/^\d{7,}/.test(s)) return false;
    // Reject common non-address notes starting with numbers
    if (/^\d+\s+(machines?|toys?|pounds?|dollars?|months?|years?|days?|weeks?|bills?|⌚)/i.test(s)) return false;
    // Must start with a number
    if (!/^\d/.test(s)) return false;
    // Must contain a street suffix or known street name
    if (/\b(st|ave?|blvd|rd|ln|lane|dr|drive|pl|place|way|ct|court|hwy|pike|tpk|turnpike|road|street|avenue|boulevard)\b/i.test(s)) return true;
    if (/\d+[-\s]+\d*\s*(beach|channel|bay|myrtle|atlantic|northern|parsons|merrick|jamaica|rockaway|flushing|union|main|cross|astoria|junction|college|woodhaven|crescent|broadway|queens|rockaway|bayview)\b/i.test(s)) return true;
    return false;
}

function parseCommission(str) {
    if (!str || !str.trim()) return 0;
    const s = str.trim().toLowerCase();
    if (s === 'tokens') return 0;
    if (s.includes('ask dan')) return 0;
    const match = s.match(/(\d+)\s*%/);
    if (match) return parseInt(match[1]) / 100;
    const match2 = s.match(/%\s*(\d+)/);
    if (match2) return parseInt(match2[1]) / 100;
    const cleaned = s.replace(/[%$]/g, '');
    const num = parseInt(cleaned);
    if (!isNaN(num) && num > 0 && num <= 100) return num / 100;
    return 0;
}

function parseDate(str) {
    if (!str || !str.trim()) return null;
    const s = str.trim().replace(/\.$/, '');
    const parts = s.split('/');
    if (parts.length < 2) return null;
    let m, d, y;
    if (parts.length === 2) {
        [m, d] = parts;
        y = '2025';
    } else {
        [m, d, y] = parts;
    }
    if (!m || !d) return null;
    let year = (y || '2025').trim();
    if (year.length === 2) year = `20${year}`;
    const mi = parseInt(m);
    const di = parseInt(d);
    const yi = parseInt(year);
    if (isNaN(mi) || isNaN(di) || isNaN(yi)) return null;
    return `${yi}-${mi.toString().padStart(2, '0')}-${di.toString().padStart(2, '0')}T12:00:00`;
}

function parseWeight(str) {
    if (!str || !str.trim()) return '0';
    const s = str.trim().replace(/\$/g, '').replace(/,/g, '.').replace(/\.$/, '').replace(/\s/g, '');
    if (/no money|no mony|closed|installed|install|machine|took all|storage|broken|empty/i.test(s)) return '0';
    const num = parseFloat(s);
    return isNaN(num) ? '0' : num.toString();
}

function parseBills(cols) {
    return {
        50: 0,
        20: parseInt(cols[0]) || 0,
        10: parseInt(cols[1]) || 0,
        5: parseInt(cols[2]) || 0,
        1: parseInt(cols[3]) || 0,
    };
}

function parseCSVLine(line) {
    const result = [];
    let current = '';
    let inQuotes = false;
    for (let i = 0; i < line.length; i++) {
        const ch = line[i];
        if (ch === '"') {
            inQuotes = !inQuotes;
        } else if (ch === ',' && !inQuotes) {
            result.push(current);
            current = '';
        } else {
            current += ch;
        }
    }
    result.push(current);
    return result;
}

function parseChangeMachine(notes) {
    let count = 0;
    let hasChange = false;
    for (const n of notes) {
        const lower = n.toLowerCase();
        if (lower.includes('change machine') || lower.includes('change mashine') || lower.includes('change mashin') || lower.includes('change mechine') || lower.includes('chengs') || lower.includes('chenge machine')) {
            hasChange = true;
            const match = lower.match(/(\d+)\s*x?\s*change/);
            if (match) count = Math.max(count, parseInt(match[1]));
            else if (lower.includes('double hopper')) count = Math.max(count, 2);
            else count = Math.max(count, 1);
        }
        if (lower.includes('token mech') || lower.includes('token machine')) {
            hasChange = true;
            count = Math.max(count, 1);
        }
    }
    return { hasChange, count: count || (hasChange ? 1 : 0) };
}

function isBusinessNameLine(str) {
    return /\s%\d+/.test(str) || /%\d+/.test(str);
}

function parseFullAddress(str) {
    const s = str.trim();
    const match = s.match(/^(.+?),\s*(.+?),\s*([A-Z]{2})\s*(\d{5})?$/i);
    if (match) {
        return { address: match[1].trim(), city: match[2].trim(), state: match[3].trim(), zip: (match[4] || '').trim() };
    }
    const match2 = s.match(/^(.+?)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\s+([A-Z]{2})\s+(\d{5})$/);
    if (match2) {
        return { address: match2[1].trim(), city: match2[2].trim(), state: match2[3].trim(), zip: match2[4].trim() };
    }
    return null;
}

// ── Main parsing loop ──
const locationGroups = [];
let currentLocation = null;

for (let i = 0; i < dataLines.length; i++) {
    const line = dataLines[i];
    if (!line.trim()) continue;

    const cols = parseCSVLine(line);
    const address = (cols[0] || '').trim();
    const name = (cols[1] || '').trim();
    const city = (cols[2] || '').trim();
    const state = (cols[3] || '').trim();
    const commission = (cols[4] || '').trim();
    const date = (cols[5] || '').trim();
    const totalWeight = (cols[6] || '').trim();
    const halfWeight = (cols[7] || '').trim();
    const user = (cols[8] || '').trim();
    const bill20 = (cols[9] || '').trim();
    const bill10 = (cols[10] || '').trim();
    const bill5 = (cols[11] || '').trim();
    const bill1 = (cols[12] || '').trim();

    // Case 1: Standard new location - address + city present
    if (isStreetAddress(address) && (city || name)) {
        if (currentLocation) locationGroups.push(currentLocation);
        currentLocation = {
            address,
            businessName: name || '',
            city: city || '',
            state: state || 'NY',
            commission,
            commissionRate: parseCommission(commission),
            notes: [],
            logs: [],
        };
        if (date) {
            currentLocation.logs.push({
                date: parseDate(date),
                totalWeight,
                halfWeight,
                user,
                bills: parseBills([bill20, bill10, bill5, bill1]),
                notes: '',
            });
        }
        continue;
    }

    // Case 2: Business name + commission line (e.g. "LUV STORY Convenience Smoke Shop %40")
    if (isBusinessNameLine(address) && !date) {
        if (currentLocation) locationGroups.push(currentLocation);
        const commMatch = address.match(/%(\d+)/);
        const commRate = commMatch ? parseInt(commMatch[1]) / 100 : 0;
        const bizName = address.replace(/%\d+/, '').trim();
        const extraNote = city; // col 2 might have a note like "Change motor 2/3/26"
        currentLocation = {
            address: '',
            businessName: bizName,
            city: '',
            state: 'NY',
            commission: commMatch ? `${commMatch[1]}%` : '',
            commissionRate: commRate,
            notes: extraNote ? [extraNote] : [],
            logs: [],
        };
        continue;
    }

    // Case 3: Full address in col 0 (e.g. "135-02 Rockaway Blvd, Jamaica, NY 11420")
    const fullAddr = parseFullAddress(address);
    if (fullAddr && !currentLocation?.address) {
        // This provides the address for a location that was started by a business name line
        currentLocation.address = fullAddr.address;
        currentLocation.city = fullAddr.city;
        currentLocation.state = fullAddr.state || 'NY';
        if (date) {
            currentLocation.logs.push({
                date: parseDate(date),
                totalWeight,
                halfWeight,
                user,
                bills: parseBills([bill20, bill10, bill5, bill1]),
                notes: '',
            });
        }
        continue;
    }

    if (fullAddr && isStreetAddress(fullAddr.address)) {
        // Standalone full address line = new location
        if (currentLocation) locationGroups.push(currentLocation);
        currentLocation = {
            address: fullAddr.address,
            businessName: '',
            city: fullAddr.city,
            state: fullAddr.state || 'NY',
            commission: '',
            commissionRate: 0,
            notes: [],
            logs: [],
        };
        if (date) {
            currentLocation.logs.push({
                date: parseDate(date),
                totalWeight,
                halfWeight,
                user,
                bills: parseBills([bill20, bill10, bill5, bill1]),
                notes: '',
            });
        }
        continue;
    }

    // Case 4: Standalone street address without city (new install entries at end)
    if (isStreetAddress(address) && !city && !currentLocation?.address?.includes(address.slice(0, 10))) {
        const addrParsed = parseFullAddress(address);
        if (currentLocation) locationGroups.push(currentLocation);
        currentLocation = {
            address: addrParsed ? addrParsed.address : address,
            businessName: '',
            city: addrParsed ? addrParsed.city : '',
            state: addrParsed ? addrParsed.state : 'NY',
            commission: '',
            commissionRate: 0,
            notes: [],
            logs: [],
        };
        if (date) {
            currentLocation.logs.push({
                date: parseDate(date),
                totalWeight,
                halfWeight,
                user,
                bills: parseBills([bill20, bill10, bill5, bill1]),
                notes: '',
            });
        }
        continue;
    }

    // Case 5: Continuation row for current location
    if (currentLocation) {
        const isEmptyRow = !address && !date && !totalWeight;
        if (isEmptyRow) continue;

        if (address) {
            currentLocation.notes.push(address);
        }

        if (date) {
            const logNote = (address && !isStreetAddress(address)) ? address : '';
            currentLocation.logs.push({
                date: parseDate(date),
                totalWeight,
                halfWeight,
                user,
                bills: parseBills([bill20, bill10, bill5, bill1]),
                notes: logNote,
            });
        }
    }
}

if (currentLocation) locationGroups.push(currentLocation);

console.log(`\n📊 Parsed ${locationGroups.length} locations from Queens CSV\n`);

for (const loc of locationGroups) {
    const displayName = loc.businessName ? `${loc.address} (${loc.businessName})` : loc.address;
    console.log(`📍 ${displayName}, ${loc.city} — ${loc.commission || 'N/A'} — ${loc.logs.length} logs`);
    for (const log of loc.logs) {
        console.log(`   📅 ${log.date} | ${log.totalWeight || '0'} lbs | by ${log.user || '?'} | $20×${log.bills[20]} $10×${log.bills[10]} $5×${log.bills[5]} $1×${log.bills[1]}`);
    }
    if (loc.notes.length > 0) {
        console.log(`   📝 Notes: ${loc.notes.join(' | ')}`);
    }
    console.log('');
}

// ── Clean up any previous Queens upload ──
console.log('🗑️  Removing existing Queens entries...');
const allDocs = await getDocs(collection(db, COLLECTION));
let deleted = 0;
for (const docSnap of allDocs.docs) {
    const data = docSnap.data();
    if (data.zone === 'Queens' || data.region === 'Queens') {
        await deleteDoc(doc(db, COLLECTION, docSnap.id));
        deleted++;
    }
}
console.log(`   Deleted ${deleted} existing Queens documents.\n`);

// ── Upload ──
console.log('🚀 Starting Firestore upload (ADDING to existing data)...\n');

let uploaded = 0;
const existingCount = (await getDocs(collection(db, COLLECTION))).size;
console.log(`   📦 Existing documents in collection: ${existingCount}\n`);

for (let i = 0; i < locationGroups.length; i++) {
    const loc = locationGroups[i];

    // Sort logs newest first
    loc.logs.sort((a, b) => {
        if (!a.date) return 1;
        if (!b.date) return -1;
        return b.date.localeCompare(a.date);
    });

    const latestLog = loc.logs[0];
    const { hasChange, count: changeMachineCount } = parseChangeMachine(loc.notes);

    // Build subtitle from important notes (red text)
    const subtitleParts = loc.notes.filter(n => {
        const lower = n.toLowerCase();
        return (
            /\d{3}.*\d{4}/.test(n) ||
            /call\s/i.test(n) ||
            /text\s/i.test(n) ||
            /close[sd]?\s/i.test(n) ||
            /no\s(bills|\$20|money)/i.test(n) ||
            /pick\s?it\s?up/i.test(n) ||
            /recommend/i.test(n) ||
            /double hopper/i.test(n) ||
            /put\s(a\s)?\$?\d+/i.test(n) ||
            /fill\sup/i.test(n) ||
            /don.t\s/i.test(n) ||
            /receipt/i.test(n) ||
            /change machine/i.test(n) ||
            /change mashine/i.test(n) ||
            /change mechine/i.test(n) ||
            /chenge machine/i.test(n) ||
            /has\schange/i.test(n) ||
            /token/i.test(n) ||
            /toy\srack/i.test(n) ||
            /gumball/i.test(n) ||
            /not\sour/i.test(n) ||
            /scale/i.test(n) ||
            /drill/i.test(n) ||
            /store\sclosed/i.test(n) ||
            /machine\s(picked|ripped|out|was off|empty|broken|cannot)/i.test(n) ||
            /re-?installed/i.test(n) ||
            /replaced/i.test(n) ||
            /fixed/i.test(n) ||
            /stuck/i.test(n) ||
            /broken/i.test(n) ||
            /shaking/i.test(n) ||
            /unplugged/i.test(n) ||
            /power/i.test(n) ||
            /burned/i.test(n) ||
            /fire/i.test(n) ||
            /hole\sin/i.test(n) ||
            /stealing|shake|broke/i.test(n) ||
            /plexiglass|glass/i.test(n) ||
            /flap/i.test(n) ||
            /motor/i.test(n) ||
            /reset/i.test(n) ||
            /plug/i.test(n) ||
            /cable/i.test(n) ||
            /come every/i.test(n) ||
            /open[s]?\s+\d/i.test(n) ||
            /sunday|monday|tuesday|wednesday|thursday|friday|saturday|wensday/i.test(n) ||
            /owner/i.test(n) ||
            /quarters/i.test(n) ||
            /new\s(install|owner)/i.test(n) ||
            /wants\smachine/i.test(n) ||
            /bigger/i.test(n) ||
            /2\smachines/i.test(n) ||
            /pac-man/i.test(n) ||
            /renovation/i.test(n) ||
            /asap/i.test(n)
        );
    });

    const subtitle = subtitleParts.join(' | ');

    const displayName = loc.businessName || loc.address;

    const customerDoc = {
        name: displayName,
        address: loc.address,
        city: loc.city,
        state: loc.state || 'NY',
        zone: 'Queens',
        region: 'Queens',
        status: 'pending',
        commissionRate: loc.commissionRate,
        hasChangeMachine: hasChange,
        changeMachineCount: changeMachineCount,
        notes: loc.notes.join('\n'),
        subtitle: subtitle,
        order: existingCount + i,
        createdAt: new Date().toISOString(),
        lastVisited: latestLog?.date || null,
        lastCollection: parseWeight(latestLog?.totalWeight),
        logNotes: latestLog?.notes || '',
        bills: latestLog?.bills || { 50: 0, 20: 0, 10: 0, 5: 0, 1: 0 },
        logs: loc.logs.map(log => ({
            date: log.date,
            collection: parseWeight(log.totalWeight),
            commissionRate: loc.commissionRate,
            bills: log.bills,
            notes: log.notes || '',
            user: log.user || '',
            totalWeight: parseWeight(log.totalWeight),
            halfWeight: parseWeight(log.halfWeight),
        })),
    };

    try {
        const docRef = await addDoc(collection(db, COLLECTION), customerDoc);
        console.log(`✅ [${i + 1}/${locationGroups.length}] ${displayName}, ${loc.city || '?'} → ${docRef.id} (${loc.logs.length} logs)${subtitle ? ' 🔴 ' + subtitle.slice(0, 60) : ''}`);
        uploaded++;
    } catch (err) {
        console.error(`❌ Failed to upload ${displayName}:`, err.message);
    }
}

console.log(`\n🎉 Done! Uploaded ${uploaded}/${locationGroups.length} Queens locations to Firestore.`);
console.log('   All locations are under zone: "Queens"');
console.log(`   Collection: "${COLLECTION}" (now has ${existingCount + uploaded} total documents)\n`);

process.exit(0);
